...
import os, logging, aiohttp, asyncio, redis, hmac, time, json, base64, httpx
...
# models.Base.metadata.create_all(bind=engine) // Database setup
...
# This endpoint receives live signals
@app.post("/_webhook_endpoint_example")  
async def webhook(request: Request, db: Session = Depends(get_db),ip: str = Depends(verify_ip_whitelist)):
...
# This function sets up a half-hour scheduler to run when no live alert comes in
async def scheduled_hf_with_delay():
    ...
    # This line pauses logic running on websocket data while the main iteration is running.
...
# This endpoint gives the ability for a general api health check
...
# This code runs when the api starts up
...
## The trade making logic - this is from a different file
